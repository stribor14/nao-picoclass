...
imlist = open(src + '/nazivi.txt', 'r').readlines()

# object sample is specified by three coordinates (row, column and size; all in pixels)
rs = [float(line.split()[1]) for line in open(src+'/anot.txt', 'r').readlines()]
cs = [float(line.split()[0]) for line in open(src+'/anot.txt', 'r').readlines()]
ss = [float(line.split()[2]) for line in open(src+'/anot.txt', 'r').readlines()]
...
for i in range(0, len(rs)):
	...
	path = src + '/pozitivneRGB/' + imlist[i].strip()
