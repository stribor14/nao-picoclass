    ...
    m_memoryProxy(getParentBroker())
{
    ...
    functionName("addClassifier", getName(), "Set parameters for classification (pass negative value to set default one)");
...
void* PicoModule::readCascade(const std::string &cascade){
    ...
    void* p_cascade;
    p_cascade = malloc(size);
    if(!p_cascade || size!=fread(p_cascade, 1, size, file)){
...
void PicoModule::addClassifier(std::string name, std::string cascade, float angle, float scalefactor, float stridefactor, int minsize, int treshold){
    ...
    temp.cascade = readCascade(cascade);
    m_classifiers.push_back(temp);
}

void PicoModule::removeClassifier(std::string name){
    for(std::list<classifier>::iterator it = m_classifiers.begin(); it != m_classifiers.end();)
        if(!name.compare(it->name)) m_classifiers.erase(it++);
        else ++it;
}
